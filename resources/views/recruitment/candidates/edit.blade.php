    ...
    @section('content_header')
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">            
                ...
                <li class="breadcrumb-item"><a href="{{ route('recruitment.candidates.form.edit',['id'=>$Candidate->id]) }}">Editar</a></li>
            </ol>
        </nav>
    @stop
        ...
        <div class="card-body  p-0">
            ...
            <form action="{{ route('recruitment.candidates.update') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row p-2">
                    ...
                    <div class="col-sm-12">
                        <x-dg-input type="text" label="Nombre" name="name" maxlength="255" value="{{$Candidate->name}}" placeholder=""  />
                    </div>
                    
                    
                    <div class="col-sm-12 col-lg-6">
                        <x-dg-input type="text" label="Email" id="email" name="email" maxlength="255" value="{{$Candidate->email}}" placeholder=""  />
                    </div>
                    
                    <div class="col-sm-12 col-lg-6">
                        <x-dg-input type="text" label="Teléfono/Celular" id="mobile_phone" name="mobile_phone" maxlength="255" value="{{$Candidate->mobile_phone}}" placeholder=""  />
                    </div>
                    ...
                    <div class="form-group col-12 col-lg-6">
                        ...
                        <select name="sources_id" id="sources_id" class="form-control" required >
                            ...
                            @foreach ($CandidateSource as $item)
                                <option {{ ($item->id==$Candidate->sources_id) ? 'selected':'' }} value="{{ $item->id }}">{{ $item->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group col-12 col-lg-6">
                        ...
                        <select name="requisition_id" id="requisition_id" class="form-control">
                            ...
                            @foreach ($Requisitions as $item)
                                <option {{ ($item->id==$Candidate->requisition_id) ? 'selected':'' }} value="{{ $item->id }}">{{ $item->branch->name }} / {{ $item->position->name }}</option>
                            @endforeach
                        </select>
                    </div>
